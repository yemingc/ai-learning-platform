...
export function CourseUnitLearnPage({
  ...
  const concepts = getConceptsByUnit(activeUnit.id, course.id);
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
      ...
      <section className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
        <div>
          <Badge variant="outline">Unit {activeUnit.sequence}</Badge>
          <h1 className="mt-5 text-4xl font-semibold leading-tight sm:text-5xl">
            {activeUnit.title} concept graph.
          </h1>
          <p className="mt-5 max-w-3xl text-base leading-7 text-muted-foreground">
            This unit is one module inside {course.title}. Students move through
            concepts, prerequisite links, static lessons, AI Teacher support, and
            learner memory before application practice.
          </p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
          ...
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              {activeUnitTopics.length} topics in {activeUnit.title}.
            </p>
          </div>
          <div className="rounded-lg border border-border bg-card p-4">
            <div className="flex items-center gap-2 text-sm font-semibold">
        ...
        <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <Badge variant="secondary">
              {activeUnit.title} - curriculum pack module
            </Badge>
